...
**New repo:** edit only the `(project_layer_mapping …)` block in this file—replace
`{{PROJECT_SLUG}}` and every `FILL_*` (see `Planner/README.md` step 4). Do not
edit other `Planner/contracts/` files for adoption.
...
(router_planner
  ...
  ; NEW REPO: fill only this block — {{PROJECT_SLUG}} and all FILL_* below; then Planner/router-state.md.
