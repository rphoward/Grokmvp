...
- `project_layer_mapping` section. Conceptual planner vocabulary
  ...
  project-specific physical layer names. In capex3: `domain → core`,
  `application → conceptual_core_boundaries` (no folder; lives inside core),
  `infrastructure → infrastructure`, `presentation → presentation`,
  plus `bootstrap` as a physical-only layer.
...
(router_planner
  ...
  (paths
    ...
    (architecture "docs/router-planner-contracts/domain-spoke-architecture.md")
    (phase_map    "docs/phase-map/PHASE-MAP.md"))
  ...
  (phase_map
    (path "docs/phase-map/PHASE-MAP.md")
  ...
  (project_layer_mapping
    ...
    (active_project capex3)

    (capex3
      (conceptual_to_physical
        (domain          core)
        (application     conceptual_core_boundaries)
      ...
      (notes
        (conceptual_core_boundaries
          "Application-layer concepts (use case orchestration, request,
           result, spoke boundary) live as conceptual boundaries inside
           the core/ folder. They are not a separate physical folder.
           Spoke boundaries may be expressed as submodules, files, or
           naming conventions within core/. The architecture_contract's
           application ownership rules still apply — they just live
           inside core/."))

      (source_of_truth
        "project_layer_mapping block in this contract, derived from stable
         project guardrails in root AGENTS.md. AGENTS.md is guardrails-only;
         it is not an active run state source."))
    ...
    (support_doc_sync
      ...
      (rule
        "Record which support docs were checked. Do not mutate support docs
         to keep active run state synchronized; active run state belongs in
         docs/router-state.md and selected RUN_DIR/STATUS.md.")
      (allowed_support_doc_writes
        (router_state_md
          "Only when the active-run pointer, active_status, active_phase,
           updated date, or selection_reason must change.")
        (context_index_md
          "Only for stable repo navigation, read-order, or topology changes;
           never for transient active run identity, current_mode,
           active_slice_file, or next_action.")
        (AGENTS_md
          "Forbidden during closeout for run tracking. Allowed only by
           explicit user-approved durable project guardrail changes outside
           active run state.")))
  ...
  (outputs
    ...
    (need_new_phase_run_dir
      "NEED_NEW_PHASE_RUN_DIR: this phase is sealed. Do not reuse the completed RUN_DIR.
       Consulting deferred_phases in docs/phase-map/PHASE-MAP.md for suggested next RUN_DIR.
