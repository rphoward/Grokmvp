...
- `router_state` packet records the single active-run pointer before per-run
...
(packet_schema
  ...
  (project_shape
    ...
    (pre_record_bootstrap
      "Multi-phase projects may pass through router phase_initialized after
       PHASE-MAP approval and before project_shape exists; create_slice_list
       records project_shape multi_phase.")
    (legacy_fallback
  ...
  (packet router_state
    ...
    (purpose "single active-run pointer for routed work")
    (required
      active_run_dir
      active_status
      active_phase
      updated)
    (authority
      "Read before any per-run STATUS.md. Completed run STATUS.md files remain
       historical records unless explicit user instruction selects one for
       repair or corrective work.")
    (selection_rule
      "explicit_user_instruction_this_session wins over router_state; otherwise
       router_state selects the active run before current_mode, active_slice_file,
       or next_action are read."))
  ...
  (packet project_layer_mapping
    ...
    (capex3_example
      ...
      (source_of_truth
        "project_layer_mapping block in codex-router-planner-unified.md,
         filled during Planner adoption and explained by Planner/AGENTS.md.
         Repository root AGENTS.md may defer to Planner/AGENTS.md or echo
         durable repo-local guardrails, but it is not an active Planner run
         state source.")))
  ...
  (packet extraction_final_closeout
    ...
    (support_doc_sync_keys
      planner_router_state
      planner_context_index
      planner_AGENTS_guardrails_only
      root_AGENTS_not_run_state
      active_run_state_fields_authoritative
      next_slice_cleanup)
  ...
  (field_contracts
    ...
    (project_shape
      ...
      (set_at      create_slice_list)
      (pre_record_bootstrap
        "phase_initialized is the only router state allowed before this field
         exists for an approved multi_phase PHASE-MAP and fresh RUN_DIR")
    ...
    (support_doc_sync
      "Closeout evidence recorded under RUN_DIR/STATUS.md that support docs
       were checked for stale active-state repetition. Active Planner run state
       is not synchronized into support docs. Planner/router-state.md owns the
       active run pointer; selected RUN_DIR/STATUS.md owns current_mode,
       active_slice_file, next_action, project_shape, and closeout_validation.
       Planner/context-index.md may change only for stable kit navigation,
       read-order, or topology updates. Planner/AGENTS.md and repository root
       AGENTS.md are guardrail files, not run ledgers, and must not be used for
       run tracking.")
